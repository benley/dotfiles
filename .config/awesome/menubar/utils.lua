-- Partially backported to Awesome 3.4 from master:
...
function lookup_icon(icon_file)
    ...
    if icon_file:sub(1, 1) == '/' and is_format_supported(icon_file) then
        ...
        -- FIXME: icon themes hax
        local icon_theme = nil -- theme.icon_theme
        if icon_theme then
        ...
        for i, directory in ipairs(icon_path) do
            if is_format_supported(icon_file) and file_exists(directory .. icon_file) then
...
function parse(file)
    ...
    if program.OnlyShowIn ~= nil and not program.OnlyShowIn:match(wm_name) then
        program.show = false
    end
